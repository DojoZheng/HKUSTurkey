...
class Maze(object):
    ...
    # turkey_img = {d: RGBA_to_RGB(join("images/", "turkey-"+d+".jpg")) for d in valid_actions}
    ...
    def __init__(self,
        ...
        if (from_file is not None) and (maze_size is None):
            with open(from_file, 'rb') as f_in:
                self.maze_data = np.genfromtxt(
                    from_file, delimiter=',', dtype=np.uint16)
        ...
        # self.turkey_img = {d:RGBA_to_RGB(join("images/", "turkey-"+d+".jpg")) for d in self.valid_actions}

    def __generate_maze(self, height=21, width=27, complexity=.25,
                        density=.25):
        """
        ...
        for i in range(density):
            x, y = random.randint(0, shape[1] // 2) * 2, random.randint(
                0, shape[0] // 2) * 2
            Z[y, x] = 1
        ...
        maze_data = np.zeros(((r - 3) // 2 + 1, (c - 3) // 2 + 1),
                             dtype=np.uint8)
        for i in range(0, r - 2, 2):
            for j in range(0, c - 2, 2):
                maze_data[i // 2, j // 2] = sum(
                    [1, 2, 4, 8][i] * ~block for i, block in enumerate(
                        np.ravel(Z[i:i + 3, j:j +
                                   3], order='F')[[3, 7, 5, 1]]))
    ...
    def __validate_maze(self):
        ...
        for r in range(height - 1):
            for c in range(width):
                if (self.maze_data[r, c] & 4 !=
                        0) != (self.maze_data[r + 1, c] & 1 != 0):
                    wall_errors.append([(r, c), 'v'])
        # Horizontally Check
        for r in range(height):
            for c in range(width - 1):
                if (self.maze_data[r, c] & 2 !=
                        0) != (self.maze_data[r, c + 1] & 8 != 0):
        ...
        if wall_errors:
            for cell, wall_type in wall_errors:
                if wall_type == 'v':
                    ...
                    print(
                        'Inconsistent vertical wall betweeen {} and {}'.format(
                            cell, cell2))
                else:
                    cell2 = (cell[0], cell[1] + 1)
                    print('Inconsistent horizontal wall betweeen {} and {}'.
                          format(cell, cell2))
    ...
    def __generate_trap(self, trap_number=5):
        ...
        destination = int(self.destination[0] * self.width +
                          self.destination[1])
        valid_range = list(range(1, destination)) + list(
            range(destination + 1, int((self.width - 1) * (self.height - 1))))
        trap_list = random.sample(valid_range, trap_number)
        self.__traps = [(ele // self.width, ele % self.width)
                        for ele in trap_list]
    ...
    def __draw_raw_maze_img(self):
        ...
        for i in range(16):
            grid_images.append(
                imageio.imread(join("images/",
                                    str(i) + ".jpg")))
        maze = np.vstack((np.hstack((grid_images[i] for i in row))
                          for row in self.maze_data))

        # Display traps and destination
        trap_img = imageio.imread(join("images", "trap.jpg"))
        dest_img = imageio.imread(join("images", "hkust_logo.jpg"))
        grid_size = 100  # default sizes for grid, trap and destination are 100
        for (r, c) in self.__traps:
            maze[r * grid_size:(r + 1) * grid_size, c * grid_size:(c + 1) *
                 grid_size, :] += trap_img + 1
        r, c = self.destination
        maze[r * grid_size:(r + 1) * grid_size, c * grid_size:(c + 1) *
             grid_size, :] += dest_img + 1
    ...
    def draw_current_maze(self):
        ...
        current_maze_img[r*grid_size:(r+1)*grid_size, c*grid_size:(c+1)*grid_size, :] += \
            self.turkey_img[self.turkey['dir']] + 1
    ...
    def is_permissible(self, location, direction):
        ...
        """
        try:
            return (self.maze_data[location] &
                    self.direction_bit_map[direction]) != 0
        except:
            print('Invalid direction or location provided!')

    def place_turkey(self, turkey_loc=None):
        """
        Place turkey into the maze, default in (0,0)
        """
        if not turkey_loc:
            turkey_loc = self.__default_turkey_loc.copy()
        self.turkey = turkey_loc
    ...
    def move_turkey(self, direction):
        """
        Move the turkey location according to its location and direction
        ...
        if self.unstable_action:
            ...
            direction = np.random.choice(
                unstable_act.keys(), p=unstable_act.values())

        if self.is_permissible(self.turkey['loc'], direction):
            self.turkey['loc'] = tuple((
                i + di
                for i, di in zip(self.turkey['loc'], self.move_map[direction])))
            self.turkey['dir'] = direction
            if self.turkey['loc'] == self.destination:
                reward = self.reward['destination']
            elif self.turkey['loc'] in self.__traps:
    ...
    def sense_turkey(self):

        return self.turkey['loc']

    def reset_turkey(self):

        self.turkey = self.__default_turkey_loc.copy()


# maze = Maze(maze_size=(6, 6), trap_number=1)
# print maze